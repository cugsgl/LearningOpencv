...
cv::Mat dtfilter(cv::Mat img, int n)
{
    //建立高斯核模板
    ...
    for (int x = 0; x < width; x++)
    {
        for (int y = 0; y < height; y++)
        {
            for (int ch = 0; ch < channel; ch++)
            {
                ...
                if (count < n * n)
                {
                    out.at<cv::Vec3b>(y, x)[ch] = (uchar)(a[n / 2] - a[0]);
                    delete[] a;
                }
                else
                {
                    out.at<cv::Vec3b>(y, x)[ch] = (uchar)(a[4] - a[1]);
                    delete[] a;
                }
...
int main(int agrc, const char *agrv[])
{
    ...
    cv::imwrite("F:\\learnOpencv\\unit2\\answer_image\\answer14.jpg", guass);
    cv::waitKey(0);
